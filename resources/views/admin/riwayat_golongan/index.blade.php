					...
					<div class="col s12">
						...
						<div class="card-content">
							...
							<div class="col s12 " style="overflow-x:auto;">
								...
								<tbody>
									@foreach($riwayat_golongan as $v)
									<tr>
										<td>{{ ($riwayat_golongan ->currentpage()-1) * $riwayat_golongan ->perpage() + $loop->index + 1 }}</td>
										<td>{{ $v->golongan }}</td>
										<td>{{ $v->pangkat }}</td>
										<td>
											@if($v->tmt_golongan && $v->tmt_golongan!='0000-00-00')
												{{ date('d-m-Y', strtotime($v->tmt_golongan)) }}
											@endif
										</td>
										<td>{{ $v->no_sk }}</td>
										<td>{{ $v->mk_tahun }}</td>
